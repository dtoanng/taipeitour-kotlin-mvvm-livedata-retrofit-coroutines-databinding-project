object BuildDependencyVersions {
    const val ANDROID_APP_LIBRARY_VERSION = "7.4.1"
    const val ANDROID_MULTIPLATFORM_VERSION = "1.8.0"

    const val  COMPOSE_VERSION = "1.3.1"
    const val  COMPOSE_ACTIVITY_VERSION = "1.6.1"

    const val  JVM_TARGET = "1.8"
    const val  KOTLIN_COMPILER_EXTENSION_VERSION = "1.4.0"
    const val KOIN = "3.2.0"

    const val  SQLDELIGHT_VERSION = "1.5.5"

    const val  COROUTINES_VERSION = "1.6.4"

    const val ANDROID_CORE_KTX = "1.10.1"
    const val ANDROID_APPCOMPAT = "1.6.1"
    const val ANDROID_MATERIAL = "1.9.0"
    const val ANDROID_NAVIGATION = "2.5.3"
    const val ANDROID_PAGING = "3.2.1"
    const val ANDROID_DATASTORE = "1.0.0"
    const val ANDROID_RECYCLE_VIEW = "1.2.1"
    const val ANDROID_SVG = "1.4"
    const val ANDROID_CONSTRAINT_LAYOUT = "2.1.4"
    const val ANDROID_RX_ANDROID = "2.1.1"
    const val ANDROID_DATA_BINDING = "8.3.1"
    const val ANDROID_LIFECYCLE = "2.8.0-rc01"

    const val RETROFIT = "2.11.0"
    const val RETROFIT_CONVERTER_GSON = "2.11.0"
    const val OKHTTP3 = "4.12.0"
    const val OKHTTP3_INTERCEPTOR = "5.0.0-alpha.2"

    const val HILT_COMPILER = "2.48"
    const val DAGGER_HILT_ANDROID = "2.48"

    const val GOOGLE_GSON = "2.10.1"
    const val GLIDE = "4.14.2"
    const val FRAGMENT_KTX = "1.5.5"
    const val RX_JAVA = "2.2.21"

    const val JUNIT = "4.13.2"
    const val EXT_JUNIT = "1.1.5"
    const val ESPRESSO = "3.5.1"
    const val ROUNDED_IMAGEVIEW = "2.3.0"
}